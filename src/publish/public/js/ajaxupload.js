...
(function (c, g) {
    ...
    var k = {
        ...
        renderHtml: function () {
            ...
            this.li = c("<li style='list-style-type:none' />").appendTo(this.AU.fileList).attr("title", name);
            a.bootstrap && (this.li = c("<a />").appendTo(this.li));
            this.prevContainer = c('<div class="ax-container" style="float:left;width:60%" />').appendTo(this.li);
            this.details = c('<div class="ax-details" style="float:left;width:40%" />').appendTo(this.li);
            this.progressInfo = c('<div class="ax-progress" style="clear:left;width:100%;margin-top:10px" />').appendTo(this.li);
            this.buttons = c('<div class="ax-toolbar" />').appendTo(this.li);
            this.prevImage = c('<img class="ax-preview" style="width:250px;" src="" alt="' + f("Preview") + '" />').appendTo(this.prevContainer);
            ...
            this.removeButton = c('<input value="Remove image" type="button">').appendTo(this.details);
            this.cropButton = c('<input value="Crop image" type="button" style="display:none;">').appendTo(this.details);
            this.progressBar = c('<div class="ax-progress-bar" style="height:2px; background-color:#3097D1" />').appendTo(this.progressInfo);
        ...
        bindEvents: function () {
            this.uploadButton.bind("click", this, function (a) {
                a.data.AU.settings.enable && (2 != a.data.status ? a.data.startUpload() : a.data.stopUpload())
            });
        ...
        doPreview: function () {
            if (this.AU.settings.previews && this.AU.hasAjaxUpload && this.file.type.match(/image.*/) && ("jpg" == this.ext || "gif" == this.ext || "png" == this.ext) && "undefined" !== typeof FileReader) {
                ...
                var e = this.prevImage,
                    d = new FileReader;
                d.onload = function (d) {
                    e.css("cursor", "pointer").attr("src", d.target.result).click(function(){openCropping(settings.formParams.block_id, settings.formParams.id)})
                };
        ...
        uploadAjax: function () {
            ...
            this.xhr.onreadystatechange = function () {
                if (4 == this.readyState && 200 == this.status) try {
                    ...
                    if (g) n.onFinishUpload(a.name, a.size, a.status, a.info, a.crop, a.image, a.block_id, a.item_id, a.width, a.height, a.token);
                    else n.currentByte = f, n.uploadAjax()
                } catch (b) {
                    n.onError(b, this.responseText, this)
                }
            };
            b = this.AU.getParams(d, c, !this.AU.useFormData);
            b.push("ax-start-byte=" + e);
            b.push("ax-last-chunk=" + g);
            if (this.AU.useFormData) {
                ...
                for(var item in a.formParams){
                    d.append(item, a.formParams[item]);
                }
        ...
        onError: function (a,json,url) {
            ...
            this.progressBar.css("width",
                "0%");
            this.uploadButton.removeClass("ax-abort");
            this.settings.error.call(this, a, this.name, json, url);
            this.settings.removeOnError && this.AU.removeFile(this.pos)
        },
        onFinishUpload: function (name, size, status, info, shouldBeCropped, image, block_id, item_id, width, height, token) {
            this.AU.slots--;
            this.name = name;
            this.status = parseInt(status);
            this.info = info;
            this.AU.hasAjaxUpload || this.AU.hasFlash || (size = this.AU.formatSize(this.size), this.sizeContainer.html(size));
            this.currentByte = 0;
            this.nameContainer.html(name);
            this.li.attr("title", name);
            this.onProgress(100);
            this.uploadButton.removeClass("ax-abort");
            this.progressPer.html(f(""));
            this.settings.success.call(this, name);
            this.progressSimulation = 0;
            this.progressBar.css("width", "0");
            name = !0;
            for (size = 0; size < this.AU.files.length; size++) 1 != this.AU.files[size].status && -1 != this.AU.files[size].status && (name = !1);
            this.AU.upload_all && this.AU.uploadAll();
            name && this.AU.finish();
            
            if(shouldBeCropped){
                this.cropButton.attr('style', "display:block");
                var $cropper = this.prevImage;
                ...
                this.cropButton.click(function(){
                    this.remove();
                    parent.progressPer.html(f("Cropping uploaded file"));
                    parent.progressSimulation = 1;
                    parent.simulateProgressBar(0);
                    $.ajax({
                        url: "/admin/settings/crop",
                        method: "POST",
                        data: {
                            block_id: block_id,
                            id: item_id,
                            img: image,
                            x: $cropper.cropper("getData").x,
                            y: $cropper.cropper("getData").y,
                            w: $cropper.cropper("getData").width,
                            h: $cropper.cropper("getData").height,
                            _token: token,
                        },
                        success: function(data){
                            console.log('cropped successfully');
                            parent.settings.removeOnSuccess && parent.AU.removeFile(parent.pos);
                            if(parent.AU.files.length == 0){
                                openSettings(block_id, 0);
                            }
                        },
                        error: function(data){
                            console.log("Cannot crop image " + image);
                            console.log(data);
                        }
                    });
                });
        ...
        onProgress: function (a) {
            ...
            this.progressPer.html(a + "%");
            if(a == 100){
                this.progressPer.html("Processing uploaded file...");
                this.progressSimulation = 1;
                this.simulateProgressBar(0);
            }
        },
        simulateProgressBar: function(a){
            if(this.progressSimulation === 0){
                return;
            }
            ...
            this.progressBar.css("marginLeft", a +"%");
            setTimeout(this.simulateProgressBar.bind(this), 100, a+10*this.progressSimulation);
        }
    }),
        t = function (a, b) {
    ...
    t.prototype = {
        ...
        renderHtml: function () {
            ...
            this.hasFlash && (this.browse_c.children(".ax-browse").remove(), e = this.container.attr("id") + "_flash", g = '\x3c!--[if !IE]> --\x3e<object style="position:absolute;width:150px;height:100px;left:0px;top:0px;z-index:1000;" id="' + e + '" type="application/x-shockwave-flash" data="' + a.flash + '" width="150" height="100">\x3c!-- <![endif]--\x3e\x3c!--[if IE]><object style="position:absolute;width:150px;height:100px;left:0px;top:0px;z-index:1000;" id="' + e + '" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"  codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0" width="150" height="100"><param name="movie" value="' + a.flash + '" />\x3c!--\x3e\x3c!--dgx--\x3e<param name="flashvars" value="instance_id=' + this.container.attr("id") + '"><param name="allowScriptAccess" value="always" /><param value="transparent" name="wmode"></object>\x3c!-- <![endif]--\x3e', this.browse_c.append('<div style="position:absolute;overflow:hidden;width:150px;height:100px;left:0px;top:0px;z-index:0;">' + g + "</div>"), this.flashObj = document.getElementById(e));
        ...
        bindEvents: function () {
            ...
            if (this.hasAjaxUpload) {
                var b = "self" == a.dropArea ? this.container[0] : c(a.dropArea)[0],
                    e = this;
                ...
                b.addEventListener("dragleave", function (b) {
                    b.stopPropagation();
                    b.preventDefault();
                    e.settings.enable && (a.dropClass ? c(this).removeClass(a.dropClass) : this.style.backgroundColor = "")
                }, !1);
        ...
        getPendingFiles: function () {
            for (var a = [], b = 0; b < this.files.length; b++) 0 == this.files[b].status && this.slots <= this.settings.maxConnections && (a.push(this.files[b]), this.slots++);
            return a
        },
        uploadAll: function () {
            this.upload_all = !0;
        ...
        getParams: function (a, b, c) {
            var d = this.settings,
                ...
                h = [];
            h.push("ax-file-path=" + (c ? encodeURIComponent(f) : f));
            ...
            h.push("ax-thumbWidth=" + d.thumbWidth);
            ...
            if ("object" == typeof a) for (var g in a) h.push(g + "=" + (c ? encodeURIComponent(a[g]) : a[g]));
            else if ("string" == typeof a && "" != a) for (c = a.split("&"), g = 0; g < c.length; g++) h.push(c[g]);
            return h
        },
