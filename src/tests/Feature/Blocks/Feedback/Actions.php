...
class Actions extends TestCase{
    ...
    public function access_item_form($assertion){
        $block = factory(Block::class)->create(['panelLocation'=>'content', 'page_id'=>1, 'name'=>'feedback'])->specify();
    ...
    public function leave_feedback_from_the_website($assertion){
        $block = factory(Block::class)->create(['panelLocation'=>'content', 'page_id'=>1, 'name'=>'feedback', 'parameters'=>'{"defaultActivation":"1","successText":"Thank you for your feedback","notify":"1"}'])->specify();
    ...
    public function edit_block_settings($assertion){
        ...
        if($assertion){
            ...
            $this->assertEquals(['id', 'defaultActivation', 'successText', 'notify', 'settingsScale', 'submit'], $block->setupForm()->names());
