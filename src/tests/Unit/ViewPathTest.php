...
class ViewPathTest extends TestCase {
    ...
    public function get_view_for_block_in_non_primary_layout(){
        ...
        $blockViewFile = resource_path('views/'.$layout->name.'/blocks/'.$block->name.'_'.$layout->class.'.blade.php');
        touch($blockViewFile);
        
        $this->assertEquals('Just.blocks.'.$block->name.'_'.$layout->class, viewPath($layout, $block));
    ...
    public function get_view_for_block_with_non_primary_class(){
        ...
        $blockViewFile = resource_path('views/'.$layout->name.'/blocks/'.$block->name.'_custom.blade.php');
        touch($blockViewFile);
        
        $this->assertEquals('Just.blocks.'.$block->name.'_custom', viewPath($layout, $block));
    ...
    public function get_view_for_block_from_primary_if_specific_file_not_exists(){
        ...
        $this->assertEquals('Just.blocks.'.$block->name, viewPath($layout, $block));
    ...
    public function exception_thrown_if_block_view_resource_not_found(){
        ...
        $block = factory(Block::class)->create(['name'=>'custom']);
        
        $exception = viewPath($layout, $block);
        
        $this->assertEquals("Exception", get_class($exception));
        $this->assertEquals('Resource "Just.blocks.'.$block->name.'" does not exists.', $exception->getMessage());
